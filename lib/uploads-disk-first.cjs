...
function resolveSafeUploadPath(uploadsRoot, requestPath) {
  ...
  // Strip /uploads prefix if present (Express mount may already strip it).
  ...
  // Reject dot segments before join (plain + odd unicode dots already decoded).
  ...
  if (st.isSymbolicLink()) {
    ...
    try {
      ...
      return { ok: false, reason: 'symlink-dangling' };
    }
    ...
    if (!st.isFile()) {
      return { ok: false, reason: st.isDirectory() ? 'directory' : 'not-file' };
    }
  ...
  return function uploadsDiskFirst(req, res, next) {
    ...
    stream.on('error', (err) => {
      ...
      if (!res.headersSent) {
        ...
        res.end(String(err.message || 'send error'));
      }
    });
    stream.pipe(res);
  };
