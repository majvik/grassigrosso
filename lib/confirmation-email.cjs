...
try {
  ...
  if (fs.existsSync(logoPath)) {
    ...
    if (fs.existsSync(distPath)) {
      logoBuffer = fs.readFileSync(distPath);
    }
  }
...
function buildConfirmationEmail(lead) {
  ...
  // Временно скрываем блок отписки в пользовательском письме без удаления логики.
        ...
        <tr>
          <td align="left" valign="middle" style="width:50%;">
            <a href="${siteUrl}" style="text-decoration:none;">
              <img src="cid:logo" alt="Grassigrosso" width="175" height="48" style="display:block;border:0;height:auto;max-width:175px;" />
            </a>
          </td>
  ...
  <tr>
    <td style="padding:40px 32px 32px;">
      ...
      <p style="margin:0;font-size:16px;line-height:1.6;color:#1e1e1e;font-family:'Nunito',Arial,Helvetica,sans-serif;">
        С&nbsp;уважением,<br>
        Команда <a href="${siteUrl}" style="color:#283e37;text-decoration:none;font-weight:600;">Grassigrosso</a>
      </p>
    </td>
  </tr>
  ...
  <tr>
    <td style="padding:20px 32px 24px;">
      <p style="margin:0;font-size:11px;line-height:1.5;color:rgba(30,30,30,0.35);font-family:'Nunito',Arial,Helvetica,sans-serif;">
        ...
        <span style="${showUnsubscribeLink ? '' : 'display:none;'}"><br>
...
С уважением,
Команда Grassigrosso ( ${siteUrl} )

---
Письмо сформировано автоматически и не требует ответа.${showUnsubscribeLink ? `\nОтписаться: ${unsubscribeUrl}` : ''}`;
