...
| `strict-schemas` | `PAGES_CMS_SCHEMA_MODE=strict` **or** download-catalog gains Task 4 fields (`title`/`lead`/`submit_label`/`catalog_pdf`/`back_label`/`back_href`) | **All** `phase2Components` + `phase2SingleTypes` (incl. extended download-catalog); full attribute contract compare; missing any → FAIL |
