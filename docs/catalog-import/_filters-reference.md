...
| `orthoFoam` | Орто-пена (в каталоге: «высокоэластичная пена», flexi, elax) |
| `memoryEffect` | С эффектом памяти |
| `nanoFoam` | Нано-пена (в каталоге: «пена повышенной плотности», карбон) |
| `forplit` | Форплит |
...
| `dualFirmness` | Разная жесткость сторон (дублирует фильтр жёсткости; автоматически для моделей с `firmness: dualFirmness`) |
