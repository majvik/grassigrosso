...
### 1. Catalog runtime cleanup

- Разрезать самые тяжёлые legacy runtime-модули каталога:
  - `src/catalog/catalog-image-modal.ts`
  - `src/catalog/catalog-filter-dom.ts`
  - затем оставшиеся связанные helper-модули вокруг selection/filter state.
- Цель: сузить прямую DOM-оркестрацию и вынести логику в более локальные React-facing adapters, не ломая текущий `data-*` и class contract.
- Важно: не переписывать каталог целиком на новый state management за один шаг; сохранять совместимость со Strapi feed, submit flow, favourites/share и mobile filters.

### 2. Shared visual layer extraction

- Для уже мигрированных React pages постепенно выносить визуальный слой из legacy global CSS в локальные CSS Modules.
- Начинать только с изолированных секций, где можно доказать parity без изменения композиции страницы.
- В первую очередь расширять:
  - `src/components/marketing/shared-page-sections.tsx`
  - локальные page components в `src/components/pages/*`
- Не трогать полноширинную spatial model страниц без явной нужды.

### 3. Catalog React surface

- Продолжать дробление React subtree каталога только по естественным зонам:
  - filter group row / select surfaces
  - favourites/send flows
  - help modal content surface
- Не возвращать giant JSX blob и не собирать новые HTML-string источники.
- Typed data в `src/components/pages/catalog-page-data.ts` можно дальше сокращать, если данные становятся реальными backend-fed структурами, но без потери fallback contract.

### 4. Legacy CSS retirement

- После подтверждённого parity по каждой page family:
  - удалять только реально мёртвые global selectors;
  - не трогать selectors, на которые ещё опирается runtime из `src/catalog/*` или shared page interactions.
- Делать это отдельными крупными пакетами, а не вперемешку с behavioural migration.

### 5. Asset contract

- Для всех новых React и HTML entry changes использовать public assets только через корневые URL `/...`, а не `./public/...`.
- `templates/marketing-page.html` уже приведён к этому контракту; не возвращать старый путь.

## Инструкция для следующего ИИ

1. Считать текущую страницу/секцию React-first, если для неё уже есть `src/components/pages/*.tsx`; не возвращаться к HTML strings.
2. Не добавлять новые container/page-shell обёртки на migrated full-width pages.
3. Для поведения использовать `data-*` hooks; не завязываться на hash-классы CSS Modules.
4. Следующий крупный технический этап:
   - сначала `src/catalog/catalog-image-modal.ts`,
   - затем `src/catalog/catalog-filter-dom.ts`,
   - затем более мелкие catalog runtime leftovers.
5. Следующий крупный визуальный этап:
   - переносить shared/full-width React sections на CSS Modules только там, где parity уже доказуем.
6. Не менять:
   - `POST /api/submit`,
   - `page` routing contract,
   - clean URL contract,
   - backend proxy contract для Strapi,
   - структуру фильтров каталога, уже согласованную с клиентом.
7. Обязательные дешёвые проверки на пакет:
   - `npm run typecheck`
   - `npm run build`
8. Визуальные проверки не автоматизировать без необходимости; просить пользователя проверить конкретные страницы после крупного UI-пакета.
