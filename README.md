...
1. **В настройках деплоя укажите:**
   ...
   - **ВАЖНО:** Убедитесь, что Node.js приложение настроено как основное приложение, а не как статический сайт
...
3. **Проверьте настройки веб-сервера:**
   - Если используется nginx/apache, убедитесь, что запросы к `/api/*` проксируются к Node.js
   - На Timeweb Cloud Apps обычно это настраивается автоматически, но если API не работает, проверьте настройки проксирования
...
4. **После деплоя:**
   ...
   - Проверьте работу API: `https://ваш-домен.twc1.net/api/test` (должен вернуть JSON)
