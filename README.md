...
Рекомендуется тип **Dockerfile** (как в репозитории): multi-stage сборка, нативный `better-sqlite3`, единый образ с `dist` и `server.cjs`.

В **корне репозитория** уже есть и попадает в Git: [`Dockerfile`](Dockerfile), [`docker-compose.yml`](docker-compose.yml) (compose — в основном для локального запуска; для App Platform достаточно Dockerfile).

### В логах деплоя только клонирование репозитория, нет сборки и запуска контейнера

Так бывает, если приложение создано с типом **Node.js** (или другим) **без** привязки к Docker, а поля **«Сборка» / «Запуск»** пустые или не соответствуют образу. Платформа тогда может ограничиться `git clone`.

**Что сделать в панели Timeweb:**

1. При создании приложения выбрать тип **Dockerfile** (в мастере — вкладка/тип «Dockerfile», не чистый «Node.js» с ручными `npm run build` / `npm start`, если вы не настраиваете их осознанно).
2. Поле **«Путь к директории проекта»** оставить **пустым**, если `Dockerfile` лежит в корне репозитория (как здесь).
3. Если интерфейс требует явные команды (зависит от тарифа/версии панели):
   - **Сборка:** из корня репозитория, например `docker build -t grassigrosso .` (имя тега может быть любым).
   - **Запуск:** например `docker run -p 3000:3000 grassigrosso` — порт **3000** должен совпадать с `EXPOSE`/`PORT` в [`Dockerfile`](Dockerfile).
4. Для сценария **docker compose** на платформе (если поддерживается): команда вроде `docker compose up` из корня, где лежит [`docker-compose.yml`](docker-compose.yml).

После корректного типа приложения в логах должны появиться шаги **build** образа и **start** контейнера, а не только clone.

**Дальше:**

1. Подключить репозиторий, ветка `main`, автодеплой по коммитам (по желанию).
2. В окружении приложения задать секреты: Telegram, SMTP, `CORS_ALLOWED_ORIGINS` (прод-домен), при необходимости очередь/антиспам.
3. Для карт на контактах: **VITE_YANDEX_MAPS_API_KEY** — в переменных **сборки** образа (`--build-arg` / build env в панели), не в `.env` репозитория.
4. Если в панели указан путь проверки состояния `/health`, а в образе задан `HEALTHCHECK` из `Dockerfile`, приоритет у **Dockerfile** ([документация Timeweb](https://timeweb.cloud/docs/apps/healthcheck-path)).

### Если деплой завершается с ошибкой

- **Лог сборки:** этап `npm run build` (Vite) или `npm ci` — нехватка памяти на тарифе, рассинхрон `package-lock.json`, ошибка компиляции `better-sqlite3` (редко при корректном Dockerfile build-stage с `python3`/`make`/`g++`).
- **`nginx.conf`:** при кастомном nginx платформа часто прогоняет `nginx -t`; некорректный синтаксис или «магические» плейсхолдеры без подстановки дают отказ до старта контейнера. В проекте upstream к Node задан как `127.0.0.1:3000` в соответствии с `Dockerfile`.
- **Healthcheck:** три неуспешных ответа на `/health` подряд или таймаут 180 с при редеплое — смотреть логи приложения, не падает ли процесс до `listen`.
- **Старый HTML на сайте:** контейнер обновился, но CDN/браузер кеширует страницу; для мета-тегов после деплоя может понадобиться время или сброс кеша.

Альтернатива без Docker (только Node): команды сборки `npm run build`, запуск `npm start` — возможны отличия в нативных модулях и в том, применяется ли ваш `nginx.conf`; для этого репозитория надёжнее образ из `Dockerfile`.

## Структура проекта

```text
├── Dockerfile             # сборка образа для продакшена (App Platform)
├── docker-compose.yml     # локально: контейнер + volume для data
├── nginx.conf             # опционально: кастомный nginx Timeweb (если подключают из репо)
