...
- **CMS hydrate не создаёт новый UI.** Наличие поля/массива в CMS-контракте не означает разрешение выводить его на странице. Hydrate может менять контент только в визуальных слотах, которые существовали в принятом baseline до CMS wiring. Новый section/card/grid/modal/CTA или перенос данных с другой страницы требует отдельного design approval пользователя.
- Перед wiring для каждой страницы зафиксировать DOM baseline: порядок верхнеуровневых секций, число code-owned карточек и критические selectors. Gate обязан проверять не только сохранение старых узлов, но и отсутствие новых consumer-зон. Формула проверки: «ровно allowlist слотов», а не «старые слоты всё ещё присутствуют».
- Поле CMS может оставаться неотрисованным на конкретной странице, если для него нет legacy-слота. Нельзя придумывать consumer только ради покрытия всех полей контракта; такое поле документируется как `merge-only / not rendered on this surface`.
...
- **Контент из локальной админки на dev/prod не попадает сам.** Рабочая БД — `strapi-catalog/.tmp/data.db` (в git нет). В git — `database/seed/data.db`, `database/seed/seed-manifest.json` (пишет `npm run strapi:sync-seed`) и `public/uploads/`. После правок: остановить Strapi → `npm run strapi:sync-seed` → **`npm run catalog:export-snapshot`** (если менялся каталог) → commit snapshot + push — **достаточно одного деплоя**: при старте контейнера `start-services.sh` сравнивает manifest с маркером на volume и копирует seed, если hash изменился (`STRAPI_AUTO_APPLY_SEED=0` отключает). `STRAPI_RESEED_ON_START=1` — принудительно (на Timeweb смена env тоже вызывает redeploy). Snapshot JSON в `public/` попадает в `dist/` при build и служит fallback при недоступности Strapi. Правки **только на сервере** в админке перезапишутся следующим push с новым seed.
