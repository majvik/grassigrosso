...
- **Контент из локальной админки на dev/prod не попадает сам.** Рабочая БД — `strapi-catalog/.tmp/data.db` (в git нет). В git — `database/seed/data.db`, `database/seed/seed-manifest.json` (пишет `npm run strapi:sync-seed`) и `public/uploads/`. После правок: остановить Strapi → `npm run strapi:sync-seed` → commit + push — **достаточно одного деплоя**: при старте контейнера `start-services.sh` сравнивает manifest с маркером на volume и копирует seed, если hash изменился (`STRAPI_AUTO_APPLY_SEED=0` отключает). `STRAPI_RESEED_ON_START=1` — принудительно (на Timeweb смена env тоже вызывает redeploy). Правки **только на сервере** в админке перезапишутся следующим push с новым seed.
