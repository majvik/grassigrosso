...
### 1. Telegram — HTML-режим + мульти-чат (`server.cjs`)
...
- `sendLeadToTelegram`: при ошибке отдельного `chat_id` пишет `console.error` — частичные сбои больше не глотаются молча
- `CHAT_ID` поддерживает несколько ID через запятую: `CHAT_ID=324152887,-5050851831` (личный чат + группа). dotenv **не перезаписывает** уже заданную переменную окружения, поэтому для локального запуска: `env -u CHAT_ID node server.cjs`
